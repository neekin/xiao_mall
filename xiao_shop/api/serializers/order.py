...
from xiao_shop.models import (
    XiaoShopingCart, XiaoShopAddress, XiaoShopOrderInfo,
    XiaoShopOrderSKU)


class XiaoShopingCartSerializer(serializers.ModelSerializer):
    """ 购物车数据序列化 """
    ...
    class Meta:
        model = XiaoShopingCart
        fields = ("id", "num", "sku", "sku_data")

    def get_sku_data(self, obj):
        sku = obj.sku
        cart_count = XiaoShopingCart.objects.filter(owner=obj.owner).count()
        return [{
            "sku_id": sku.id,
            "spu_id": sku.spu.id,
            "title": sku.spu.title,
    ...
    def create(self, validated_data):
        ...
        carts = XiaoShopingCart.objects.filter(owner=self.context["request"].user, sku=validated_data['sku'])
        if carts.exists():
...
class XiaoShopAddressSerializer(serializers.ModelSerializer):
    """ 收货地址序列化 """
    ...
    def create(self, validated_data):
        ...
        if is_default:
            address = XiaoShopAddress.objects.filter(is_default=True, owner=self.context["request"].user)
            if address.exists:
...
class XiaoShopOrderInfoSerializer(serializers.ModelSerializer):
    """订单序列化
    """
    owner = serializers.HiddenField(
        default = serializers.CurrentUserDefault())
    ...
    def get_alipay_url(self, obj):
        ...
        #     notify_url=xiao_shop_settings.ALIPAY.get('NOTIFY_URL')     # 可选, 不填则使用默认notify url
        # )
        # if settings.DEBUG:
