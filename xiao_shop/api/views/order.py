...
from xiao_shop.models import (
    XiaoShopingCart, XiaoShopAddress, XiaoShopOrderInfo, XiaoShopOrderSKU, 
    XiaoShopSKU)
from xiao_shop.api.serializers.order import (
    XiaoShopingCartSerializer, XiaoShopAddressSerializer,
    XiaoShopOrderInfoSerializer)
from xiao_shop.api.permissions import IsOwnerOrReadOnly


class XiaoShopingCartViewsets(viewsets.ModelViewSet):
    """购物车接口
    """
    # 必须登录，并且只返回当前登录用户的数据
    permission_classes = [IsAuthenticated, IsOwnerOrReadOnly]
    serializer_class = XiaoShopingCartSerializer
    authentication_classes = [SessionAuthentication]

    def get_queryset(self):
        return XiaoShopingCart.objects.filter(owner=self.request.user, is_del=False)
    ...
    def create(self, request, *args, **kwargs):
        ...
        # 重复加入购物车，则进行更新操作
        carts = XiaoShopingCart.objects.filter(owner=request.user, sku=serializer.validated_data['sku'])
        if carts.exists():
...
class XiaoShopAddressViewsets(viewsets.ModelViewSet):
    """收货地址接口
    """
    permission_classes = [IsAuthenticated, IsOwnerOrReadOnly]
    serializer_class = XiaoShopAddressSerializer
    authentication_classes = [SessionAuthentication]

    def get_queryset(self):
        queryset = XiaoShopAddress.objects.filter(is_del=False, owner=self.request.user)
        return queryset
...
class XiaoShopOrderInfoViewSet(viewsets.ModelViewSet):
    """订单列表，需要登录
    list:
        订单列表，只可查看自己的订单
    create:
    ...
    """
    permission_classes = [IsAuthenticated, IsOwnerOrReadOnly]
    serializer_class = XiaoShopOrderInfoSerializer
    authentication_classes = [SessionAuthentication]

    def get_queryset(self):
        queryset = XiaoShopOrderInfo.objects.filter(is_del=False, owner=self.request.user)
        return queryset
    ...
    def create(self, request, *args, **kwargs):
        ...
        <QueryDict: {
            ...
            'carts': ['[{"id":33,"num":1,"sku":1,"sku_data":[{"sku_id":1,"spu_id":1,"title":"Apple iPhone 13 Pro (A2639) 128GB 远峰蓝色 支持移动联通电信5G 双卡双待手机",
                ...
                "sell_price":13,"stocks":100,"main_picture":"/XiaoShop/sku/cdb86550185fe4b7.jpg","sku_total":"13.00","cart_count":1}]}]'], 
            'pay_method': ['2']}>
        """
        """后端需要的数据格式
        ...
        order_info = XiaoShopOrderInfo.objects.filter(id=id, order_sn=order_sn, owner=request.user).first()
        # 思考：这里的商品数量应该获取购物车对应的数量，那这里依然应该以购物车为基准
        for cart in self.get_carts(team_datas['carts']):
            sku = XiaoShopSKU.objects.get(id=cart['sku'])
            print(sku.sell_price)
            XiaoShopOrderSKU.objects.create(order=order_info, sku=sku, count=cart['num'], price=sku.sell_price)
        # 删除已经流转到订单的购物车数据
        XiaoShopingCart.objects.filter(id__in=carts_ids).delete()
        return Response(serializer.data, status=status.HTTP_201_CREATED, headers=headers)
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        XiaoShopOrderInfo.objects.filter(id=id).update(pay_status=4)
        return super().partial_update(request, *args, **kwargs)


class XiaoShopPayViewSet(mixins.CreateModelMixin, viewsets.GenericViewSet):
    """立即支付
    create:
        创建立即支付订单
    """
    ...
    def create(self, request, *args, **kwargs):
        ...
        data['address'] = f"""\
            ...
            """.strip().replace(" ", "")
        data['freight'] = 0   # 运费
        sku = XiaoShopSKU.objects.get(id=sku_list[0]['sku_id'])
        ...
        id = serializer.data['id']
        order_sn = serializer.data['order_sn']
        order_info = XiaoShopOrderInfo.objects.filter(
            id=id, order_sn=order_sn, owner=request.user).first()
        XiaoShopOrderSKU.objects.create(
