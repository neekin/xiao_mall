...
class XiaoShopCategory(BaseModelMixin):
    """商品分类模型
    """
    ...
    def get_navs(cls):
        ...
        from xiao_shop.utils import generate_tree
...
class XiaoShopBrand(BaseModelMixin):
    """商品品牌模型
    """
    categories = models.ManyToManyField(
        XiaoShopCategory, blank=True, verbose_name="所属分类", related_name="brand_cates", help_text="多对多")
    name = models.CharField("品牌名称", max_length=50)
    desc = models.CharField("品牌描述", max_length=100, blank=True, default="")
    logo = models.ImageField(
        "品牌logo",
        upload_to="XiaoShop/brand/",
...
class XiaoShopSPU(BaseModelMixin):
    """商品SPU
    ...
    main_picture = models.ImageField("商品主图", upload_to="XiaoShop/spu/", max_length=200)
    ...
    category = models.ManyToManyField(XiaoShopCategory, related_name="spu_cates", verbose_name="商品分类")
    brand = models.ForeignKey(XiaoShopBrand, on_delete=models.CASCADE, verbose_name="商品品牌")
...
class XiaoShopSKU(BaseModelMixin):
    """ 商品SKU """
    spu = models.ForeignKey(XiaoShopSPU, on_delete=models.CASCADE, related_name="skus", verbose_name="商品")
    options = models.ManyToManyField('XiaoShopSPUSpecOption', blank=True, verbose_name="规格值")
    main_picture = models.ImageField(
        "商品主图", upload_to="XiaoShop/sku/", max_length=200)
...
class XiaoShopSPUSpec(BaseModelMixin):
    """ 商品规格选项 """
...
class XiaoShopSPUSpecOption(BaseModelMixin):
    """商品的规格值
        SKU将与这个表建立一对多的关联关系
        一个SKU可以有多个规格值
        例如：一件衣服拥有颜色 和 大小等多个规格值
    """
    spec = models.ForeignKey(XiaoShopSPUSpec, on_delete=models.CASCADE, related_name="specs", verbose_name="规格")
...
class XiaoShopSpecToOption(BaseModelMixin):
    """商品规格的关系表
    ...
    spec = models.ManyToManyField(XiaoShopSPUSpec, verbose_name="规格")
...
class XiaoShopSPUCarousel(CarouselModelMixin):
    """ 商品轮播图 """
