...
from .goods import XiaoShopSKU

User = get_user_model()


class XiaoShopOrderInfo(BaseModelMixin):
    """订单信息"""
...
class XiaoShopOrderSKU(BaseModelMixin):
    """订单商品"""
    order = models.ForeignKey(XiaoShopOrderInfo, on_delete = models.PROTECT, verbose_name="订单")
    sku = models.ForeignKey(XiaoShopSKU, on_delete = models.PROTECT, blank=True, null=True, verbose_name="订单商品")
...
class XiaoShopAddress(BaseModelMixin):
    """收货地址
    """
...
class XiaoShopingCart(BaseModelMixin):
    # 购物车数据模型
    owner = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="用户")
    sku = models.ForeignKey(XiaoShopSKU, on_delete=models.CASCADE, verbose_name="商品sku")
