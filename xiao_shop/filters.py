...
from xiao_shop.models import XiaoShopCategory, XiaoShopSPU, XiaoShopSKU, XiaoShopBrand


class XiaoShopSPUFilter(django_filters.FilterSet):
...
class XiaoShopSKUFilter(django_filters.FilterSet):
    """商品分类页筛选
    """
    order = django_filters.OrderingFilter(fields=('sell_price',), label='排序', field_labels={'sell_price':'售价'},)
    spu__brand = django_filters.ModelChoiceFilter(field_name='spu__brand', queryset=XiaoShopBrand.objects.filter(is_del=False), label="品牌")
    ...
    spu__category = django_filters.ModelChoiceFilter(
        ...
        queryset=XiaoShopCategory.objects.filter(parent__isnull=False, is_del=False), 
        label="分类",)
